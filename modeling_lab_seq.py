...
def feedforward_adapter(input_tensor, hidden_size=4, init_scale=1e-3):
  ...
  with tf.variable_scope("adapters"):
    re_input_tensor = tf.reshape(input_tensor,[-1,128])
    in_size = re_input_tensor.get_shape().as_list()[1]
    w1 = tf.get_variable(
        "weights1", [in_size, 4],
    ...
    b1 = tf.get_variable(
        ...
        collections=["adapters", tf.GraphKeys.GLOBAL_VARIABLES])
    net = tf.tensordot(re_input_tensor, w1, [[1], [0]]) + b1
